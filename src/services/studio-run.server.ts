...
async function runSingleChartGA4(
  ...
  try {
    ...
    const out: any[] = rows.map((r: any) => {
      ...
      // GA4 ritorna date come 'YYYYMMDD' — la UI nostra già le mostra così.
      return { [dim]: dVal, [metric]: mVal };
    });

    return { id: chart.id, data: out };
  } catch (err: any) {
    // Rientriamo SEMPRE per non bloccare la dashboard
    return {
      id: chart.id,
      data: [],
...
router.post("/api/studio/run", async (req: Request, res: Response) => {
  try {
    ...
    // Se manca property → mock per ogni chart
    if (!propertyId) {
