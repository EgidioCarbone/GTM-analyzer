...
> = {
  ...
  eventCount: ["eventi", "eventi totali", "total events", "event count"],
...
function extractMetrics(t: string): Array<{ y: IntentResult["charts"][number]["y"]; label: string }> {
  ...
  for (const [metric, syns] of Object.entries(METRIC_SYNONYMS)) {
    for (const s of syns) {
      ...
      if (rex.test(t)) {
        ...
        break; // passa alla prossima metrica
      }
    }
  }

  // Se non trovassi nulla, di default “page view”
  if (!found.length) {
    found.push({ y: "screenPageViews", label: "page view" });
  }
...
function chooseDimension(t: string): IntentResult["charts"][number]["x"] {
  const txt = " " + t + " ";
...
router.post("/api/studio/intent", async (req: Request, res: Response) => {
  try {
    ...
    const x = chooseDimension(prompt);
    const metrics = extractMetrics(prompt); // <-- più metriche
    const rangeHintDays = detectRangeHintDays(prompt);

    // Crea un chart per ogni metrica richiesta
