      ...
      <main className="flex-1 p-6 space-y-6 bg-gray-50 overflow-y-auto">
        ...
        {container && page === "tags" && (
          <TagSection
            ...
            updateTags={(newTags) => setContainer({ ...container, tag: newTags })}
          />
        )}
        ...
        {/* “plan” e “testing” da implementare in seguito */}
    ...
    <aside className="w-64 bg-[#1a365d] text-white py-6 space-y-2 shrink-0">
      ...
      {links.map(({ id, label, icon: Icon }) => (
        <button
          ...
          disabled={disabled}
          className={`w-full flex gap-2 items-center px-5 py-2 transition ${current === id ? "bg-white/10" : ""
            } ${disabled
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-[#1a365d]">🚀 GTM Analyzer</h1>
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            ID: <strong>{containerId}</strong> – Versione:{" "}
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow space-y-2">
          <h2 className="text-lg font-semibold">📌 Metriche rapide</h2>
      ...
      {/* qualità */}
      <div className="bg-white p-4 rounded-xl shadow space-y-2">
        <h2 className="text-lg font-semibold">🩺 Qualità del container</h2>
        <ul className="list-disc pl-5 text-sm text-gray-700 leading-6">
...
export function TagSection({ tags, triggers, updateTags }: {
  tags: any[];
  triggers: any[];
  ...
  const groups: { id: string; label: string; emoji: string; data: any[] }[] = [
  ...
  const hasTriggers = (tag: any) => (tag.triggerId ?? []).some((id: any) => triggers.some(tr => tr.triggerId === id));
  ...
  const explain = (item: any) => `🔖 Tag tipo "${item.type}" con trigger ${item.triggerId?.join(", ") ?? "–"}`;
  ...
  return (
    <>
      {groups.map(({ id, label, emoji, data }) =>
        !!data.length ? (
          <div key={id} className="mb-4 border rounded-xl bg-white shadow">
            <button
              className="w-full px-4 py-3 text-left flex justify-between items-center font-semibold text-[#1a365d]"
              onClick={() => setOpen(open === label ? null : label)}
            >
              <span>{emoji} {label}</span>
              {open === label ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </button>
              ...
              <div className="divide-y">
                {data.map((t: any, i: number) => (
                  <div key={i} className="flex justify-between items-center p-4">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setDetail(t)} className="px-3 py-1.5 rounded bg-[#1a365d] text-white text-sm flex items-center gap-1">
                        <Info className="w-4 h-4" /> Dettagli
                      </button>
                      <button onClick={() => alert(explain(t))} className="px-3 py-1.5 rounded bg-[#FF6B35] text-white text-sm flex items-center gap-1">
                        <Sparkles className="w-4 h-4" /> AI
                      </button>
                      <button onClick={() => handleDelete(t)} className="px-3 py-1.5 rounded bg-red-600 text-white text-sm flex items-center gap-1">
      ...
      {detail && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white max-w-3xl w-full max-h-[80vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            ...
            <table className="w-full text-sm">
              <tbody>
                {Object.entries(detail).map(([key, value]) => (
                  <tr key={key} className="border-t">
                    <td className="font-semibold py-2 pr-4 align-top whitespace-nowrap">{key}</td>
                    <td className="py-2 break-words">
                      <pre className="whitespace-pre-wrap break-words bg-gray-100 p-2 rounded">
                        {JSON.stringify(value, null, 2)}
                      </pre>
            ...
            <div className="text-right mt-6">
              <button onClick={() => setDetail(null)} className="px-4 py-2 bg-[#1a365d] text-white rounded hover:brightness-110 text-sm">
                Chiudi
              </button>
            </div>
          </div>
      ...
      {errorMsg && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-md p-6 rounded shadow-xl text-center space-y-4">
            <p className="text-gray-700 text-sm whitespace-pre-wrap">{errorMsg}</p>
            <button onClick={() => setErrorMsg(null)} className="px-4 py-2 bg-[#1a365d] text-white rounded hover:brightness-110 text-sm">
              Chiudi
            </button>
          </div>
        </div>
