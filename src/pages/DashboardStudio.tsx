...
export default function DashboardStudio(): JSX.Element {
  const [propertyId] = React.useState<string | null>(
    new URLSearchParams(location.search).get("property") ||
      localStorage.getItem("ga4:propertyId") ||
      null
  );
  ...
  async function handleExecute() {
    try {
      ...
      const resIntent = await fetch("/api/studio/intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt }),
      });
      if (!resIntent.ok) throw new Error(await resIntent.text());
      const j = await resIntent.json();
      const intentCharts: ChartSpec[] =
        Array.isArray(j?.charts) && j.charts.length ? j.charts : parsed.charts;

      const skeletons: LocalChart[] = intentCharts.map((c, i) => ({
        id: c.id || `chart_${i}`,
      ...
      const body = {
        propertyId,
        startDate: parsed.days
          ? defaultRange(parsed.days).startDate
          : range.startDate,
        endDate: parsed.days
          ? defaultRange(parsed.days).endDate
          : range.endDate,
        charts: skeletons.map(({ id, title, type, x, y, y2 }) => ({
          id,
          title,
          type,
          x,
      ...
      const resRun = await fetch("/api/studio/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      if (!resRun.ok) throw new Error(await resRun.text());
      const data = await resRun.json();
      const byId: Record<string, any[]> = {};
      for (const c of data?.charts ?? [])
        byId[c.id] = Array.isArray(c.data) ? c.data : [];
      setCharts((prev) => prev.map((c) => ({ ...c, data: byId[c.id] || [] })));
    } catch (e: any) {
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {SUGGESTIONS.map((s) => (
                    <button key={s} onClick={() => setPrompt(s)} className="text-xs px-2 py-1 rounded-full border bg-white hover:bg-slate-50 shadow-sm">{s}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            {share.url ? (
              <div>
                ...
                <div className="mt-3 flex gap-2">
                  <button onClick={async () => { try { await navigator.clipboard?.writeText(share.url!); setCopyOk(true); setTimeout(() => setCopyOk(false), 2000); } catch {} }} className="px-3 py-2 rounded border">
