...
export default function GA4Chat() {
  ...
  // UI-only state for conversational cards and view controls (does not affect business logic)
  type Card = {
    id: number;
    queryText: string;
    narrative: string;
  ...
  // Append a new card when narrative updates after a request
  ...
  // Parse first markdown table from narrative
  function parseFirstMarkdownTable(md:string): { headers: string[]; rows: string[][] } | null {
    ...
    for (let i=0;i<lines.length;i++) {
      ...
      if (l.startsWith('|') && l.endsWith('|')) {
        ...
        if (/^\|\s*[-:]+\s*(\|\s*[-:]+\s*)+\|$/.test(sep)) { start = i; break; }
      }
    }
  ...
  // Minimal bar chart with plain divs (no new deps)
  ...
  function handleDownload(card:Card) {
    ...
    if (table) {
      const csv = [table.headers.join(','), ...table.rows.map(r=>r.map(v=>`"${(v||'').replace(/"/g,'""')}"`).join(','))].join('\n');
  ...
  function handleSaveView(card:Card) {
    // TODO: integrare salvataggio vista su backend quando disponibile
    ...
    <div className="max-w-5xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="text-center space-y-3">
        <div className="mx-auto h-12 w-12 rounded-full bg-indigo-600/10 text-indigo-600 flex items-center justify-center">
          <svg viewBox="0 0 24 24" fill="currentColor" className="h-6 w-6"><path d="M12 2l1.9 5.7h6L15 11.3 16.9 17 12 13.7 7.1 17 9 11.3 4.1 7.7h6L12 2z"/></svg>
        </div>
        <h1 className="text-3xl font-bold tracking-tight text-gray-900">Il tuo analista AI per i dati GA4</h1>
        <p className="text-gray-600 max-w-2xl mx-auto">Fai domande in linguaggio naturale e ottieni insight immediati dai tuoi dati Analytics 4.</p>
      </div>
      {/* Input pill */}
      <div className="sticky top-0 z-10 bg-gray-50/80 backdrop-blur supports-[backdrop-filter]:bg-gray-50/60 py-2">
        <div className="rounded-full border border-gray-200 bg-white shadow-sm px-4 py-2 flex items-center gap-3">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-5 w-5 text-gray-400"><path d="M21 15v4a2 2 0 0 1-2 2H7l-4 3V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          <input
            className="flex-1 bg-transparent placeholder:text-gray-400 focus:outline-none text-gray-900"
            value={query}
            onChange={(e)=>setQuery(e.target.value)}
            placeholder="Inserisci la tua domanda sui dati (es. sessioni ultime 2 settimane per device)"
          />
          <button
            onClick={ask}
            disabled={loading}
        ...
        {loading && (
          <div className="mt-2 text-xs text-gray-600 flex items-center gap-2">
            <svg className="h-4 w-4 animate-spin text-indigo-600" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path></svg>
      ...
      {narrative && (
        <div className="hidden rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          {range && (
            <>
            <p className="text-sm text-gray-500 mb-3 hidden">
              Periodo: {range.startDate} - {range.endDate}
            </p>
            <p className="mb-4 text-xs text-gray-600">
              <span className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-1 font-medium text-gray-700">Periodo: {range.startDate} - {range.endDate}</span>
            </p>
            </>
          )}
      ...
      <div className="space-y-6 mt-6">
        ...
        {cards.length === 0 && !loading && (
          <div className="rounded-xl border border-dashed border-gray-200 bg-white p-10 text-center text-gray-500">Invia una domanda per vedere i risultati qui sotto.</div>
        )}
      </div>
