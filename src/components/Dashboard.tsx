...
const Dashboard: React.FC = () => {
  ...
  const containerName = ((container as any)?.name) || ((container as any)?.containerId) || 'Container non disponibile';
        ...
        if ((gtmMetrics?.distribution?.html ?? 0) > ((gtmMetrics?.counts?.tags ?? 0) * 0.1)) {
          alerts.push({ type: 'warning', icon: '⚠️', message: 'Troppi tag HTML custom rilevati' });
        }
        ...
        return (
          <main className="p-6 space-y-6 bg-slate-50 min-h-screen dark:bg-slate-950">
            ...
            <ErrorBoundary fallback={<div className="rounded-2xl border border-red-200 dark:border-red-800 bg-white dark:bg-gray-900 p-6 shadow-sm"><p className="text-red-700 dark:text-red-300 text-sm font-medium">Errore nel calcolo della qualità del container. Riprova più tardi.</p></div>}>
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">Interventi consigliati per massimizzare affidabilità e copertura dati.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
              <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm flex flex-col">
                ...
                <div className="rounded-2xl p-4 bg-slate-50 dark:bg-slate-900/40" style={{ height: 260 }}>
              ...
              <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm flex flex-col justify-between">
                ...
                <div className="grid grid-cols-1 gap-3 text-sm">
                  {[
                    ...
                    const details = (gtmMetrics.actionPlan ?? []).map((it) => it).filter(Boolean);
                    return (
                      <div key={item.type} className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 bg-slate-50 dark:bg-slate-900/40 group relative">
                        <div className="flex items-center gap-3">
                          <div>
                            <p className="font-medium text-slate-900 dark:text-white">{item.label}</p>
                            <p className="font-bold text-lg text-slate-900 dark:text-white">{safeRender(item.value)}</p>
                            {item.subtitle && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{details.map((d) => safeRender((d as any).type)).join(', ')}</p>}
                          </div>
                        </div>
                      </div>
                    );
